...
import { canAccessUpload, canPublicAccessUpload, resolveUploadFilePath } from "../../lib/upload-access.js";
...
export async function servePublicUploadController(
  req: AuthRequest,
  res: Response,
  next: NextFunction,
): Promise<void> {
  try {
    ...
    const allowed = await canPublicAccessUpload(uploadPath);
