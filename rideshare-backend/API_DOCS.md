...
### GET `/drivers/me/vehicles` 🔒 _(alias)_
...
Search available trips by route and date. No auth required.
...
```
> `callbackUrl` and `returnUrl` are optional — Paychangu uses them to redirect after payment.
**Response `201`:**
```json
{
  "paymentId": "uuid",
